...
HDR = {"apikey": SUPABASE_KEY, "Authorization": f"Bearer {SUPABASE_KEY}", "Content-Type": "application/json", "Prefer": "return=representation"}
...
def ler_contrato_ia(img_bytes, media_type="image/jpeg"):
    if not ANTHROPIC_KEY:
        return None, "Chave Anthropic não configurada nos Secrets"
    b64 = base64.b64encode(img_bytes).decode("utf-8")
    prompt = """Analise o contrato de transporte e retorne APENAS JSON puro sem markdown:
{"cliente":"AUTOPORT ou SADA ou VIX etc","motorista":"NOME EM MAIUSCULAS","placa":"ABC1234","frota":"","contrato":"numero","data":"DD/MM/AAAA","fat_bruto":0.0,"chapa":0.0,"destino":"Cidade/UF","qtd_veiculos":0,"dt_pagamento":"DD/MM/AAAA ou vazio"}"""
    ...
    bloco = {"type": "document" if is_pdf else "image", "source": {"type": "base64", "media_type": media_type, "data": b64}}
    try:
        r = requests.post("https://api.anthropic.com/v1/messages", headers=hdrs,
            json={"model": "claude-sonnet-4-20250514", "max_tokens": 1000,
                  "messages": [{"role": "user", "content": [bloco, {"type": "text", "text": prompt}]}]}, timeout=30)
        if not r.ok:
            return None, f"Erro API: {r.status_code} — {r.text[:200]}"
...
if aba == "📊 Dashboard":
    ...
    if df.empty:
        ...
        c3.metric("⚠️ Pendentes", len(df[df["status"].isin(["ABERTO", "PENDENTE"])]))
    ...
    with st.expander("📷 Importar por Foto/PDF (IA)", expanded=False):
        upl = st.file_uploader("Foto ou PDF do contrato", type=["jpg", "jpeg", "png", "webp", "pdf"])
        if upl:
            if upl.type != "application/pdf":
                st.image(upl, use_container_width=True)
            else:
                st.success(f"📄 PDF carregado: {upl.name}")
            if st.button("🤖 Ler com IA", use_container_width=True):
                with st.spinner("Analisando..."):
                    dados, erro = ler_contrato_ia(upl.read(), upl.type)
                if erro:
                    st.error(f"Erro: {erro}")
                elif dados:
                    st.session_state["ia"] = dados
                    st.success("✅ Contrato lido! Campos preenchidos abaixo.")
                    st.rerun()
        if not ANTHROPIC_KEY:
            st.warning("⚠️ Configure ANTHROPIC_KEY nos Secrets do Streamlit.")
    ...
    with st.form("fnovo", clear_on_submit=True):
        ...
        if fat > 0:
            a, f = com(mot, fat)
            st.info(f"💳 {'Sem adiantamento · ' if mot in SEM else f'Adiantamento: {R(a)} · '}Folha: {R(f)}")
        if st.form_submit_button("✅ Salvar Contrato", use_container_width=True):
            if not cont or not fat:
                st.error("Preencha os campos obrigatórios!")
            else:
                novo = {"id": str(uuid.uuid4()), "motorista": mot, "cliente": cli, "placa": placa, "frota": frota, "contrato": cont, "data": str(data_v), "fat_bruto": fat, "chapa": chapa, "destino": dest, "qtd_veiculos": int(qtd), "adiantamento_pago": adpago, "dt_pagamento": str(dt_pag) if dt_pag else None, "status": sts, "obs": obs}
    ...
    if not dv.empty:
        if busca:
            b = busca.upper()
            dv = dv[dv.apply(lambda r: b in str(r.get("motorista", "")).upper() or b in str(r.get("contrato", "")).upper() or b in str(r.get("destino", "")).upper(), axis=1)]
    ...
    if not dv.empty:
        ...
        st.download_button("📥 Exportar CSV", csv, f"contratos_{periodo.replace('/','_')}.csv", "text/csv")
        if perm == "total":
            st.divider()
            st.markdown("#### ✏️ Editar / Excluir")
            labels = [f"{r['motorista']} — {r['contrato']} — {fd(str(r['data'])[:10])}" for _, r in dv.iterrows()]
            sel = st.selectbox("Selecione", labels)
            row = dv.iloc[labels.index(sel)]
            with st.expander("✏️ Editar"):
                with st.form("fedit"):
                    ec1, ec2 = st.columns(2)
                    es = ec1.selectbox("Status", STATUS, index=STATUS.index(row.get("status", "ABERTO")) if row.get("status") in STATUS else 0)
    ...
    if df.empty:
        ...
        c2.metric("Faturamento", R(fat))
        c3.metric("Adiantamento", R(a) if mot not in SEM else "N/A")
        c4.metric("Folha", R(f))
        c5.metric("Prêmio", "🏆 Elegível!" if fat >= META else f"Falta {R(META - fat)}")
        st.divider()
        if not dm.empty:
    ...
    if df.empty:
        ...
        for mot in MOTORISTAS:
            ...
            fat = dm["fat_bruto"].sum()
            a, f = com(mot, fat)
            rows.append({"Motorista": mot, "Tipo": "10% folha" if mot in SEM else "5%+5%", "Viagens": len(dm), "Faturamento": R(fat), "Adiantamento": "N/A" if mot in SEM else R(a), "Folha": R(f), "Chapas": R(dm["chapa"].sum()), "Total": R(a + f)})
            ...
            with st.expander(f"{'🏆' if el else '⏳'} {mot} — {R(fat)}", expanded=el):
                ...
                if perm == "total" and el:
                    with st.form(f"fp_{mot}"):
                        ...
                        ps = pc1.selectbox("Status", STATUS_P, index=STATUS_P.index(prem.get("status", "QUALIFICADO")) if prem.get("status") in STATUS_P else 0)
